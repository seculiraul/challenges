...
/*
    1. Take each number in the interval and square it
    2. If i equal 1 then add it to the result
    3. Take middle of the length of the squared number and split the square nr in 2 parts. One from 0 to middle and from middle to end
    4. Add the two numbers and check if there are equal to the inital nr
 */
...
function kaprekarNumbers(p, q) {
  ...
  const res = []
  for (let i = p; i <= q; i++) {
    const x = Math.pow(i, 2)
    if (i === 1) {
      res.push(i)
    }
    const middle = Math.floor(x.toString().length / 2)
    const l = parseInt(x.toString().slice(0, middle))
    const r = parseInt(x.toString().slice(middle))
    if (l + r === i) {
      res.push(i)
    }
  }
